(function( $ ) {
	...
	$(document).ready(function(){
		...
		$('.wpmediacategory-filter').val(mediasorterConfig3.current_folder);
	              	...
	              	success: function (res){
	              		if(res.success){
	              			...
	              			if ($('.wpmediacategory-filter').val() != null) {
			              		...
			              		if (length == 0) {
				              		$('.wp-list-table tbody').append(mediasorterConfig3.no_item_html);
				              		$('.displaying-num').hide();
				              	} else {
				              		$('.displaying-num').text(length + ' ' + (length == 1 ? mediasorterConfig3.item : mediasorterConfig3.items));
				              	}
	              			}
	              		}
		...
		function themedo_move_1_attachment(event, des_folder_id){
			...
			jQuery.ajax({
              type : "POST",
              dataType: 'json',
              data : {id: attachment_id, action: 'mediasorterGetTermsByAttachment', nonce: mediasorterConfig3.nonce},
              url : ajaxurl,
