...
var themedo_mediasorter_free 	= true;
var themedo_mediasorter_pro_link = 'http://mediasorter.frenify.com/1/';
...
(function ($){
    ...
    var MediamaticCoreTree = {
		...
		pluginUrl:		mediasorterConfig.pluginUrl,
		...
		createActions: function(){
			var html = '<div class="mediasorter_actions_content"><ul><li class="kaku"><span class="add_new">'+mediasorter_translate.new_folder+'</span></li><li><span class="rename">'+mediasorter_translate.rename+'</span></li><li><span class="delete">'+mediasorter_translate.delete+'</span></li></ul></div>';
			$('body').append(html);
		},
		...
		addActionsContent: function(){
			...
			$('.menu-item .action_button').on('click', function(e){
				...
				var content = $('.mediasorter_actions_content');
				var li 		= el.parent();
				var id 		= li.data('id');
			...
			$(window).on('click', function(){
				$('.mediasorter_actions_content').removeClass('active').attr('data-id', '');		 
			});
		...
		imgToSvg: function(){
			$('img.mediasorter_be_svg').each(function(){
				var $img 		= $(this);
        ...
        themedo_mediasorter_upgrade_options: function () {
            var options = {
                title: mediasorter_translate.notice,
                html: mediasorter_translate.limit_folder,
                type: "warning",
                showCancelButton: true,
                confirmButtonText: mediasorter_translate.ok,
                cancelButtonText: mediasorter_translate.no_thank,
		...
		fireActions: function(){
			...
			// add new
			$('.mediasorter_actions_content span.add_new').on('click', function(){				
              	self.doInsertFolder();
			});
			
			// rename
			$('.mediasorter_actions_content span.rename').on('click', function(){
			...
			// delete
			$('.mediasorter_actions_content span.delete').on('click', function(){
			...
			var html = '<div id="mediasorter_be_confirm">';
					html += '<div class="confirm_inner">';
						html += '<div class="desc_holder">';
							...
							html += '<p>' + mediasorter_translate.error_occurred + '</p>';
						html += '</div>';
						html += '<div class="links_holder">';
        ...
        newBehavior: function () {
            ...
            $(document).on('click', '.add-new-folder-now', function (event) {
                ...
                if (name == '') {
                    alert(mediasorter_translate.folder_name_enter);
                } else {
                    var parent = 0;
                    var depth = 0;
            ...
            $(document).on('click', '#themedo-mediasorter-defaultTree .jstree-anchor', function (event) {
                event.preventDefault();
            ...
            $('#themedo-mediasorter-folderTree .jstree-anchor').dblclick(function (e) {
                e.preventDefault();
                setTimeout(function () {
                    $('.js_mediasorter_rename').trigger('click');
                }, 100);
            });
            $('.js_mediasorter_rename').on('click', function () {
        ...
        doRefreshFolder: function (folder_id) {
            if ($.trim(folder_id)) {
                var data = {
                    'action': 'mediasorterAjaxRefreshFolder',
        ...
        doInsertFolder: function () {
            ...
            if (!$('#folders-to-edit #menu-item-' + self.current_folder).length) {
                self.current_folder = null;
            ...
            if (self.current_folder == null) {
                ...
                if (depth >= (self.options.globalMaxDepth + 1)) {
                    ...
                    if (e.length == 0) {
						...
						//var mainlist = $('#folders-to-edit');
						//mainlist.append(self.newFolderFormTemplate());
						
                        $(self.newFolderFormTemplate('menu-item-depth-' + depth)).insertAfter($('#folders-to-edit #menu-item-' + self.current_folder));
						self.imgToSvg();
        ...
        newFolderFormTemplate: function (extra_class) {
			...
			return '<li class="new-folder-wrap mediasorter_new_and_edit ' + extra_class + '"><div class="input_holder"><img src="'+self.pluginUrl+'/assets/img/folder.svg" class="mediasorter_be_svg" /><input type="text" name="add-new-folder-name" value="" class="folder-input add-new-folder-name" id="" autocomplete="off" /></div><div class="action_buttons"><span class="add-new-folder-now mf_confirm"><img src="'+self.pluginUrl+'/assets/img/check.svg" class="mediasorter_be_svg" /></span><span class="add-new-folder-cancel mf_cancel"><img src="'+self.pluginUrl+'/assets/img/cancel.svg" class="mediasorter_be_svg" /></span></div></li>';
        ...
        editFolderFormTemplate: function (id, val, extra_class) {
			...
			return '<li data-id="' + id + '" class="edit-folder-wrap mediasorter_new_and_edit ' + extra_class + '"><div class="input_holder"><img src="'+self.pluginUrl+'/assets/img/folder.svg" class="mediasorter_be_svg" /><input type="text" name="edit-folder-name" value="' + val + '" class="folder-input edit-folder-name" id="" autocomplete="off" /></div><div class="action_buttons"><span class="edit-folder-now mf_confirm"><img src="'+self.pluginUrl+'/assets/img/check.svg" class="mediasorter_be_svg" /></span><span class="edit-folder-cancel mf_cancel" ><img src="'+self.pluginUrl+'/assets/img/cancel.svg" class="mediasorter_be_svg" /></span></div></li>';
        },
        ...
        newFolderTemplate: function (id, name, parent, depth) {
            ...
            return '<li id="menu-item-' + id + '" data-id= "' + id + '" class="menu-item menu-item-depth-' + depth + '">' +
				...
				'<img src="'+self.pluginUrl+'/assets/img/folder.svg" class="mediasorter_be_svg" />'+
