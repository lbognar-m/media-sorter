...
var mediasorterWMC = {
	...
	mediasorter_begin_loading: function(){
		jQuery('.mediasorter_be_loader').addClass('loading');
	},

	mediasorter_finish_loading: function(){

		jQuery('.mediasorter_be_loader.loading').removeClass('loading').addClass('finish');
		setTimeout(function(){ 
			 jQuery('.mediasorter_be_loader.finish').removeClass('finish');
		}, 400);
	},

	mediasorterWMCgetBackboneOfMedia: function(obj) {
	...
	updateCount: function(term_from, term_to){

		//console.log('List Mode Test');
	...
	updateCountAfterDeleteFolder: function(deleted_count){
